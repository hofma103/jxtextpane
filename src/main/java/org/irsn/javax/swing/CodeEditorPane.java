...
public class CodeEditorPane extends LineNumbersTextPane {
		...
		public void updateHighlights() {
			...
			if (caret_line) {
				if (caret_line_start >= 0) {
					...
					try {
						highlight_width = getHighlighter().addHighlight(caret_line_start, caret_line_end,
								painter_width);
						nh++;
			...
			if (caret_brace) {
				if (caret_brace_start >= 0) {
					...
					try {
						highlight_brace = getHighlighter().addHighlight(caret_brace_start, caret_brace_end,
								painter_brace);
						nh++;
					} catch (BadLocationException ex) {
						ex.printStackTrace();
	...
	public CodeEditorPane() {
		...
		completionMenu.addKeyListener(new KeyAdapter() {
			...
			@Override
			public void keyPressed(KeyEvent e) {// To handle cycling of
												// suggestions with completion
												// key
					...
					if (visible_completion_keywords != null && visible_completion_keywords.size() > 0) {
						try {
							...
							MenuSelectionManager.defaultManager()
									.setSelectedPath(((KeyWordItem) completionMenu.getComponents()[0]).path);
	...
	public class KeyWordItem extends JMenu implements Comparable {// TODO: help
																	// content
																	// may be
																	// displayed
																	// automatically
		...
		public KeyWordItem(final String name, final Map<String, String> dictionnary, JPopupMenu parent,
				final int alreadywriten) {
	...
	public void paint(Graphics g) {
		...
		if (vertical_line > 0) {
			...
			g.drawLine(getVisibleRect().x + xline, getVisibleRect().y, getVisibleRect().x + xline,
	...
	protected void updateDocumentFilter() {
		((AbstractDocument) this.getDocument())
				.setDocumentFilter(new DocumentFilterChain(braceCompleter, syntaxDocumentFilter, blockDocumentFilter));
	}
