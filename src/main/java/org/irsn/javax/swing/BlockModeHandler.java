...
/**
 * DocumentFilter to support block mode selection. Selection highlighting is
 * overloaded with custom one.
 * 
...
public class BlockModeHandler extends DocumentFilter implements ClipboardOwner {
				...
				if (isBlockModeKeyEvent(e)) {
					...
					if (isBlockMode()) {
						...
						if (component.getHighlighter().getHighlights().length > 0) {
							...
							end = component.getHighlighter()
									.getHighlights()[component.getHighlighter().getHighlights().length - 1]
											.getEndOffset();
	...
	/*
	 * void printActions() { System.err.println("ACTIONS:"); for (Object object
	 * : component.getActionMap().keys()) { System.err.println(object + " " +
	 * component.getActionMap().get(object)); }
	 * System.err.println("======================="); }
	 */
	void resetPaste() {
	...
	public String getSelectedBlock() {
		...
		if (isBlockMode() && (selections != null && selections.length > 1)) {
			...
			try {
				...
				for (int i = 0; i < cnt - tailHighlights; i++) {// do not use
																// the last
																// highlight
																// dedicated to
																// highlight
	...
	public void pasteAsBlock(String s) {
		...
		if (isBlockMode()) {
			...
			if (selections != null && selections.length > 1) {
				setBlockMode(false);// needed to not apply block mode insert
									// which duplicate string for each
									// selections
				String[] lines = s.split("\n");
				try {
					int cnt = selections.length;
					for (int i = 0; i < cnt - tailHighlights; i++) {// do not
																	// use the
																	// last
																	// highlight
																	// dedicated
	...
	@Override
	public void insertString(DocumentFilter.FilterBypass b, int offs, String str, AttributeSet a)
			throws BadLocationException {
		if (component == null) {
			return;
		}
		Highlighter.Highlight[] selections = component.getHighlighter().getHighlights();
		if (!(offs == 0 && 0 == component.getDocument().getLength()) && isBlockMode()
				&& (selections != null && selections.length > 1)) {
			try {
				int cnt = selections.length;
				...
				if (selectInsert) {// if not, it means that insert is not
									// related to selection... (like in
									// insrtString( O, ...))
					Arrays.sort(start);
	...
	public void remove(DocumentFilter.FilterBypass b, int offs, int len) throws BadLocationException {
		...
		Highlighter.Highlight[] selections = component.getHighlighter().getHighlights();
		if (!(offs == 0 && len == component.getDocument().getLength()) && isBlockMode()
				&& (selections != null && selections.length > 1)) {
			try {
				int cnt = selections.length;
				int start[] = new int[cnt - tailHighlights];
				...
				if (selectRemove) {// if not, it means that insert is not
									// related to selection... (like in
									// insrtString( O, ...))
					Arrays.sort(start);
					Arrays.sort(end);
					for (int i = start.length - 1; i >= 0; i--) {
						// System.err.println(start[i] + " .. " + (end[i] -
						// start[i]));
	...
	@Override
	public void replace(DocumentFilter.FilterBypass b, int offset, int length, String text, AttributeSet attrs)
			throws BadLocationException {
		if (component == null) {
			return;
		}
	...
	private void super_replace(DocumentFilter.FilterBypass b, int offset, int length, String text, AttributeSet attrs)
			throws BadLocationException {
		if (component == null) {
			return;
		}
		...
		// try {
		/*
		 * if (filter != null) { filter.replace(getFilterBypass(), offset,
		 * length, text, attrs); }
		 */
		// else {
		if (length > 0) {
	...
	/**
	 * @param BlockMode
	 *            true for block mode selection, false for original multiline
	 *            selection.
	 */
	...
	public class BlockHighlightPainter extends LayeredHighlighter.LayerPainter {
		...
		public Shape paintLayer(Graphics g, int offs0, int offs1, Shape bounds, JTextComponent c, View view) {
			...
			if (offs0 == view.getStartOffset() && offs1 == view.getEndOffset()) {
				...
				// System.err.println(offs0 + " " + view.getStartOffset() + " "
			...
			try {
				int luoffset = c.viewToModel(new Point(Math.min(c.modelToView(offs0).x, c.modelToView(offs1).x),
						Math.min(c.modelToView(offs0).y, c.modelToView(offs1).y)));
				int rloffset = c.viewToModel(new Point(Math.max(c.modelToView(offs0).x, c.modelToView(offs1).x),
						Math.max(c.modelToView(offs0).y, c.modelToView(offs1).y)));
