...
/**
 * This DocumentFilter supports syntax dependant colorization. RegExp are
 * available if RegExpHashMap is used to give KeyWords Yous should consider to
 * simplify this class for better performance.
 */
public class DefaultSyntaxColorizer extends SyntaxColorizer {
	...
	// TODO setKeywordBackgroundColor, setKeywordItalic, setKeywordBold,
	// setKeywordUnderline
	...
	@Override
	public void insertString(DocumentFilter.FilterBypass b, int offset, String str, AttributeSet a)
	...
	@Override
	public void replace(DocumentFilter.FilterBypass b, int offset, int length, String str, AttributeSet as)
			throws BadLocationException {
	 ...
	 */
	private int startingMultiLineComment(String content, int startOffset, int endOffset) throws BadLocationException {
	 ...
	 */
	private int endingMultiLineComment(String content, int startOffset, int endOffset) throws BadLocationException {
	...
	protected int getOtherToken(String content, int startOffset, int endOffset) {
		if (isStartNumber(content.charAt(startOffset)) && (startOffset == 0
				|| !(isLetter(content.charAt(startOffset - 1)) || isNumber(content.charAt(startOffset - 1))))) {// to
																												// avoid
																												// sdfgdfg<.>654654
			if (endOffset > startOffset + 1) {
				if (isNumber(content.charAt(startOffset + 1))) {// to avoid
																// 65465454<.>rsdgdrg
					int endOfToken = startOffset + 1;
					while (endOfToken < endOffset && isNumber(content.charAt(endOfToken))) {
						endOfToken++;
					}
					if (endOfToken == endOffset || !isLetter(content.charAt(endOfToken))) {// to
																							// avoid
																							// 65465<4>zertert
	 ...
	 * { StringBuffer whiteSpace = new StringBuffer(); int line =
	 * rootElement.getElementIndex(offset); int i =
	 * rootElement.getElement(line).getStartOffset();
	 * 
	 * while (true) { String temp = doc.getText(i, 1);
	 * 
	 * if (temp.equals(" ") || temp.equals("\t")) { whiteSpace.append(temp);
	 * i++; } else { break; } }
	 * 
	 * return "{\n" + whiteSpace.toString() + "\t\n" + whiteSpace.toString() +
	 * "}"; }
